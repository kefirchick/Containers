...
class set {
 public:
  ...
  class iterator {
   public:
    ...
    value_type operator*() {
      if (!ptr) throw std::out_of_range("ERROR: set is empty");
      return ptr->val;
    }
    ...
    iterator operator++() {
      ...
      throw std::out_of_range("ERROR: end of set");
      return *this;
    }
    ...
    iterator operator--() {
      ...
      throw std::out_of_range("ERROR: begin of set");
      return *this;
    }
  };
 ...
 public:
  ...
  explicit set(std::initializer_list<value_type> const &list) : set() {
    for (auto element : list) {
      insert(element);
    }
  }

  set(const set &s) : _size(0), _root(nullptr) { add_nodes(s); }

  set(set &&s) : _size(0), _root(nullptr) {
    add_nodes(s);
    s.clear();
  }
  ...
  iterator begin() const {
    if (!_root) throw std::out_of_range("ERROR: set is empty");
    node *current = _root;
    while (current->left) {
      current = current->left;
  ...
  std::pair<iterator, bool> insert(const value_type &value) {
    if (contains(value)) std::make_pair(end(), false);
    iterator it;
    if (!_root) {
